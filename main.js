...
var S3Client = class {
  ...
  async createDirectory(parentDir, folderName) {
    ...
    const objectKey = basePrefix ? `${basePrefix}/${dirClean}${folderName}/.keep` : `${dirClean}${folderName}/.keep`;
    try {
      const params = new URLSearchParams({ "X-Amz-Expires": "3600" });
...
var CloudAttachView = class extends ItemView {
  ...
  showNewFolderDialog() {
    if (!this.client) {
      new Notice(t("view.no_account"), 3e3);
      return;
    }
    ...
    confirmBtn2.onclick = async () => {
      ...
      if (!name) {
        new Notice(t("view.new_folder_name_empty"), 3e3);
        return;
      }
      if (name.includes("/")) {
        new Notice("\u26A0\uFE0F " + t("view.new_folder_failed", { error: "\u540D\u79F0\u4E0D\u80FD\u542B /" }), 4e3);
        return;
      }
      confirmBtn2.disabled = true;
      new Notice(t("view.new_folder_creating"), 2e3);
      try {
        ...
        if (result.ok) {
          ...
          new Notice(t("view.new_folder_failed", { error: result.error || "unknown" }), 5e3);
          confirmBtn2.disabled = false;
        }
      } catch (e) {
        new Notice(t("view.new_folder_failed", { error: e.message }), 5e3);
        confirmBtn2.disabled = false;
      }
    };
...
module.exports = class CloudAttachPlugin extends Plugin {
  ...
  _showPdfOverlay(url) {
    ...
    nameSpan.textContent = "\u{1F4C4} " + fileName;
      ...
      if (zoomScale === null) {
        ...
        for (let i = ZOOM_STEPS.length - 1; i >= 0; i--) {
          if (ZOOM_STEPS[i] < autoScale - 1e-3) {
            zoomScale = ZOOM_STEPS[i];
            break;
          }
        }
        if (zoomScale === null)
          zoomScale = ZOOM_STEPS[0];
      } else {
        const idx = ZOOM_STEPS.indexOf(zoomScale);
        if (idx > 0)
          zoomScale = ZOOM_STEPS[idx - 1];
      }
      updateZoomLabel();
      reRender();
    };
    ...
    pageInput.onchange = () => {
      ...
      if (v && v >= 1 && v <= (pdfPromise._numPages || 1))
        scrollToPage(v);
    };
    pageInput.onkeydown = (ev) => {
      if (ev.key === "Enter")
        pageInput.blur();
    };
    ...
    nextBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg>';
    nextBtn.onclick = () => {
      if (currentPage < (pdfPromise._numPages || 1))
        scrollToPage(currentPage + 1);
    };
    ...
    const buildThumbnailPanel = async (pdf) => {
      ...
      handle.onpointerdown = (ev) => {
        ...
        const onMove = (e2) => {
          const nw = sw + (e2.clientX - sx);
          if (nw >= 100 && nw <= 500)
            thumbnailPanel.style.width = nw + "px";
        };
        const onUp = () => {
          gripLine.style.background = "var(--text-muted)";
          document.body.style.cursor = "";
    ...
    const updateZoomLabel = () => {
      if (zoomScale !== null)
        ...
        zoomLabel.textContent = "\u9AD8";
    };
    const renderAllPages = async (pdf) => {
      const totalPages = pdf.numPages;
      const firstPg = await pdf.getPage(1);
      const firstVp = firstPg.getViewport({ scale: 1 });
      firstPageW = firstVp.width;
      firstPageH = firstVp.height;
      let scale;
      if (zoomScale !== null) {
        scale = zoomScale;
      } else if (zoomMode === "fit-width") {
    ...
    const applyViewMode = () => {
      ...
      scrollEl.style.flexWrap = "";
      scrollEl.style.justifyContent = "";
      scrollEl.style.alignItems = "";
      scrollEl.style.gap = "";
    ...
    const bindScroll = (totalPages) => {
      ...
      scrollEl.onscroll = () => {
        ...
        for (const c of canvases) {
          ...
          if (top >= 0 && top < ch * 0.6) {
            const pn = parseInt(c.dataset.pageNum, 10);
            if (pageInput.value !== String(pn)) {
              pageInput.value = String(pn);
              currentPage = pn;
              highlightThumbnail(pn);
            }
            break;
          }
        }
    ...
    const scrollToPage = (pageNum) => {
      if (!pdfPromise._numPages || pageNum < 1 || pageNum > pdfPromise._numPages)
        return;
  ...
  async _renderHeicAsImage(imgEl, url) {
    ...
    try {
      await new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error("timeout")), 8e3);
        imgEl.onload = () => {
          clearTimeout(timer);
          resolve();
        };
