...
case "$MANIFEST_VERSION" in
  *.dev*)
    cat >&2 <<'BLOCKED'
⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔
⛔  禁止！manifest.json 是 .dev 版本
⛔
⛔  AI 助手请按以下步骤操作：
⛔  1. 修改 src/main.js → 只改源码
⛔  2. npm run build
⛔  3. 执行 deploy.sh → 脚本自动管理版本号
⛔
⛔  禁止手动改 manifest.json 版本号
⛔  规则：~/.qclaw/skills/garry-personal/SKILL.md
⛔  → §1.3 deploy 规范 + §1.4 版本号
⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔
BLOCKED
    exit 1
    ;;
...
if [ "$MODE" = "dev" ]; then
  ...
  if [ -d "$OBSIDIAN_PLUGIN_DIR" ]; then
    echo "==> 同步文件到 Obsidian 插件目录"
    cp "$SCRIPT_DIR/main.js" "$OBSIDIAN_PLUGIN_DIR/main.js"
    [ -f "$SCRIPT_DIR/heic2any.bundle.js" ] && cp "$SCRIPT_DIR/heic2any.bundle.js" "$OBSIDIAN_PLUGIN_DIR/heic2any.bundle.js" || true
    [ -f "$SCRIPT_DIR/styles.css" ] && cp "$SCRIPT_DIR/styles.css" "$OBSIDIAN_PLUGIN_DIR/styles.css" || true
  else
...
# ----------------------------------------------------------
# 7. GitHub Release（仅发布模式，由 GitHub Actions 自动处理）
# ----------------------------------------------------------
if [ "$MODE" = "release" ]; then
