## v0.4.480.dev - 2026-09-03

- ✏️ 优化：重命名弹窗默认选中文件名主体（不含后缀），文件夹及 .xxx 隐藏文件仍全选
- 🌐 修复：OpenList `/d/` 签名链接路径恢复保留中文（仅空格/特殊字符编码），与插入预览显示一致
- 🔧 修复：OpenList 插入链接改用 `/d/` 永久代理链接并附带 sign 签名（解决 token 型后端链接过期及 401），无 sign 时回退原直链（v0.4.478 历史记录，保留）
- 🖼️ 优化：PDF 预览隐藏 Obsidian 放大镜按钮（仅 PDF；图片与 `</>` 按钮保持默认）
- 🧹 优化：卸载时清理监听器，防止累积
- 🙏 特别感谢 [congyong](https://github.com/congyong) 的 PR #1，贡献 `/d/` 永久链接的修复思路

## v0.4.477.dev - 2026-08-16

- 🔧 优化：onunload 清理 PDF 嵌入按钮移除监听（防累积），移除放大镜补 console.log

## v0.4.476.dev - 2026-08-16

- 🔧 修复：PDF 预览隐藏 Obsidian 放大镜按钮（第 3 版：一次性删除防连锁，保留 `</>`）

## v0.4.475.dev - 2026-08-16

- 🔧 修复：PDF 预览隐藏 Obsidian 放大镜按钮（第 2 版：扩大容器范围 + MutationObserver 兜底）
...
- 修复：_bindScroll 放大(>1x)才边界翻页拦截，缩小/自适应走原生 snap
...
- 修复：连续模式翻页乱跳 — _scrollToPage 改用真实坐标定位替代估算的 clientHeight*idx
- 修复：连续模式页码/尺寸不一致 — 补上 _sizeCanvas 确保连续模式每页尺寸统一

## v0.4.229.dev - 2026-07-03

- 修复：适应宽度底部被裁 — _sizeCanvas 超限时改为按高度适配
- 修复：放大模式无 Y 轴滚动 — 放大时 wrap 不设 overflow，由 scrollEl 统一滚动
- 修复：适应高度居左 — 宽幅 canvas 左对齐+横向滚动，避免居中裁切

## v0.4.227.dev - 2026-07-03

- 移除：双页浏览模式（极简，减少 bug 面，保留单页和连续两种）
...
- 全屏预览 overhaul：将 PdfFullscreenView 完整 UI 搬到 _showPdfOverlay 中
  - position:fixed overlay 覆盖整个视口（无 Space 隔离问题）
  - 完整工具栏：文件名、缩略图面板（可拖拽）、缩放按钮(0.25x-5x)、视图模式下拉（连续/单页/双页）、页码输入、翻页、版本角标
  - 缩放逻辑：手动 10 档 + 适应宽度/高度 + 缩放度标签
  - 缩略图面板：异步构建不阻塞主渲染，拖拽调整宽度，点击跳页高亮
  - 滚轮翻页（单页/双页模式），连续模式原生滚动
  - Esc / 关闭按钮退出
