...
## [0.3.082.dev] - 2026-06-05

### 修复
- **工具栏简化**：移除分隔符和全屏按钮，恢复 v0.3.042 样式（absolute 定位右上角）
- **宽度解析修复**：确保 `![]()` 语法正确解析宽度属性（支持 `![|500]`、`![](url){width=500}`、`<img width="500">`）
- **语法支持**：同时支持 `![]()` 和 `{% pdf url %}` 两种语法
