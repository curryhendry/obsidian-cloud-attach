## v0.4.019.dev - 2026-06-25

- 修复：自动上传加重试（gif/mov 写入较慢时 500ms 不匹配再等 1s×2）

## v0.4.018.dev - 2026-06-25

- 修复：提取附件前先剔除代码块内容（inline/fenced code），避免代码示例中的附件语法被误识别

## v0.4.017.dev - 2026-06-25

- 修复：提取附件时过滤空路径，避免 `![]()` 空语法被识别为第4个附件

## v0.4.016.dev - 2026-06-25

- 修复：自动上传正则路径前缀改为可选（兼容有无路径前缀两种写法）
- 修复：调试日志 ext 变量在声明前使用导致 ReferenceError
- 清理：删除 cancel dead key、调试日志，按钮文案改为"确认启用"

## v0.4.015.dev - 2026-06-25

- 修复：自动上传正则路径前缀改为可选（兼容有无路径前缀两种写法）
- 修复：调试日志 ext 变量在声明前使用导致 ReferenceError

## v0.4.013.dev - 2026-06-25

- 新增：自动上传功能（高级设置），开启后粘贴/拖入附件自动用默认账号上传
- 优化：自动上传设置改用 Obsidian 原生 Toggle 组件
- 优化：弹窗确认路径显示在独立框内，按钮统一确定在左
- 优化：移除所有弹窗取消按钮（用右上角 X 关闭替代）
- 修复：Toggle 确定后不回弹（confirmed flag 管控 onClose）
- 修复：mp4 自动上传正则不匹配带路径前缀文件名
- i18n：中英文文案同步更新
